...
class RawEvent(BaseModel):
    ...
    log_source = Column(String(100), nullable=False, index=True)  # 'auth.log', 'apache.log', 'suricata', etc.
...
class NormalizedEvent(BaseModel):
    ...
    # Classification
    event_type = Column(String(100), nullable=True, index=True)  # 'login', 'file_access', 'network_connection', etc.
    severity = Column(String(20), nullable=True)  # 'Low', 'Medium', 'High', 'Critical'
    description = Column(Text)
    
    # Relationships
    raw_event_id = Column(BigInteger, ForeignKey("raw_events.id"), nullable=True, index=True)
    case_id = Column(Integer, ForeignKey("cases.id"), nullable=True, index=True)
