...
class Evidence(BaseModel):
    ...
    )  # 'Verified', 'Tampered', 'Pending Verification'
    ...
    uploader = relationship(
        "User", foreign_keys=[uploaded_by], back_populates="uploaded_evidence"
    )
    chain_of_custody = relationship(
        "ChainOfCustody", back_populates="evidence", cascade="all, delete-orphan"
    )
    yara_results = relationship(
        "YaraResult", back_populates="evidence", cascade="all, delete-orphan"
    )
    volatility_results = relationship(
        "VolatilityResult", back_populates="evidence", cascade="all, delete-orphan"
    )
    pcap_analysis = relationship(
        "PCAPAnalysis", back_populates="evidence", cascade="all, delete-orphan"
    )
    browser_artifacts = relationship(
        "BrowserArtifact", back_populates="evidence", cascade="all, delete-orphan"
    )
...
class ChainOfCustody(BaseModel):
    ...
    )  # 'uploaded', 'viewed', 'analyzed', 'exported', 'hash_verified', 'report_generated'
