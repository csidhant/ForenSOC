...
class CaseCRUD:
    ...
    def get_cases_by_creator(
        db: Session, creator_id: int, skip: int = 0, limit: int = 10
    ) -> List[Case]:
        """Get all cases created by a user."""
        return (
            db.query(Case)
            .filter(Case.created_by == creator_id)
            .offset(skip)
            .limit(limit)
            .all()
        )
...
class CaseNoteCRUD:
    ...
    @staticmethod
    def create_note(
        db: Session, case_id: int, note_text: str, analyst_id: Optional[int] = None
    ) -> CaseNote:
        """Create a new case note."""
        note = CaseNote(case_id=case_id, analyst_id=analyst_id, note_text=note_text)
