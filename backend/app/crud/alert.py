...
class AlertCRUD:
    ...
    def get_all_alerts(
        ...
        return query.order_by(Alert.detected_time.desc()).offset(skip).limit(limit).all()
    ...
    @staticmethod
    def get_unreviewed_alerts(db: Session, skip: int = 0, limit: int = 50) -> List[Alert]:
        """Get all unreviewed alerts."""
        return db.query(Alert).filter(Alert.status == "New").order_by(Alert.detected_time.desc()).offset(skip).limit(limit).all()
    
    @staticmethod
    def get_alerts_by_severity(db: Session, severity: str, skip: int = 0, limit: int = 50) -> List[Alert]:
        """Get alerts by severity."""
        return db.query(Alert).filter(Alert.severity == severity).order_by(Alert.detected_time.desc()).offset(skip).limit(limit).all()
    
    @staticmethod
    def get_alerts_by_creator(db: Session, creator_id: int, skip: int = 0, limit: int = 50) -> List[Alert]:
        """Get all alerts created by a user."""
        return db.query(Alert).filter(Alert.created_by == creator_id).order_by(Alert.detected_time.desc()).offset(skip).limit(limit).all()
    
    @staticmethod
    def get_alerts_assigned_to(db: Session, user_id: int, skip: int = 0, limit: int = 50) -> List[Alert]:
        """Get all alerts assigned to a user."""
        return db.query(Alert).filter(Alert.assigned_to == user_id).order_by(Alert.detected_time.desc()).offset(skip).limit(limit).all()
...
class AlertNoteCRUD:
    ...
    @staticmethod
    def get_alert_notes(db: Session, alert_id: int, skip: int = 0, limit: int = 50) -> List['AlertNote']:
        """Get all notes for an alert."""
        from app.models.alert import AlertNote
        return db.query(AlertNote).filter(AlertNote.alert_id == alert_id).offset(skip).limit(limit).all()
    
    @staticmethod
    def create_note(db: Session, alert_id: int, note_text: str, analyst_id: Optional[int] = None) -> 'AlertNote':
