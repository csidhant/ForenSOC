...
from app.models.forensics import BrowserArtifact # Re-using this or could create a FileArtifact model
...
SUSPICIOUS_EXTENSIONS = {
    ".exe", ".dll", ".ps1", ".bat", ".vbs", ".scr", ".pif", ".com", ".msi",
    ".js", ".jse", ".wsf", ".wsh", ".hta", ".cpl", ".msc", ".jar"
}

ARCHIVE_EXTENSIONS = {".zip", ".rar", ".7z", ".tar", ".gz"}

def analyze_file(db: Session, evidence_id: int, analyzed_by: Optional[int] = None) -> Dict[str, Any]:
    ...
    if is_suspicious_ext:
        findings["recommendations"].append(f"Suspicious extension {ext} detected. Perform YARA scan.")
    if is_mismatch:
        findings["recommendations"].append(f"File type mismatch: {file_type} with extension {ext}.")
